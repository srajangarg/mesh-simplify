...
#ifndef __DGP_MatrixMN_hpp__
#define __DGP_MatrixMN_hpp__
...
class /* DGP_DLL_LOCAL */ MatrixMNBase : public AddressableMatrix<T>
{
...
class /* DGP_DLL_LOCAL */ SquareMatrixN : public MatrixMNBase<N, N, T>
{
  private:
 ...
 */
template <long N, typename T>
class /* DGP_API */ MatrixMN<N, N, T> : public Internal::SquareMatrixN<N, T>
{
  private:
...
template <long M, long N, typename T>
DGP::VectorN<N, T>
operator*(DGP::VectorN<M, T> const & v, DGP::MatrixMN<M, N, T> const & m)
{
  DGP::VectorN<N, T> result;
  for (long j = 0; j < N; ++j)
  {
    result[j] = m(0, j) * v[0];
...
template <long M, long N, typename T>
DGP::MatrixMN<M, N, T>
operator*(T const & t, DGP::MatrixMN<M, N, T> const & m)
{
  return m * t;
}
