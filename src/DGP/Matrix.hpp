...
#ifndef __DGP_Matrix_hpp__
#define __DGP_Matrix_hpp__
...
class /* DGP_API */ Matrix : public AddressableMatrix<T>, public ResizableMatrix<T>
{
  private:
  ...
  public:
    ...
    static Matrix fromDiagonal(std::vector<T> const & diagonal)
    {
    ...
    Matrix(MatrixT const & src, typename std::enable_if< std::is_base_of< AddressableMatrix<typename MatrixT::Value>,
     ...
     */
    template <typename MatrixT> explicit
    Matrix(MatrixT const & crm, typename std::enable_if< std::is_base_of< CompressedRowMatrix< typename MatrixT::Value,
    ...
    Matrix(MatrixT const & ccm,
           typename std::enable_if< std::is_base_of< CompressedColumnMatrix< typename MatrixT::Value,
                                                                             typename MatrixT::Index2D,
                                                                             typename MatrixT::Index1D >,
                                                     MatrixT >::value >::type * dummy = NULL)
    : num_rows(0), num_cols(0), values(NULL), owns_memory(true)
    ...
    Matrix & operator=(Matrix const & src)
    {
      ...
      fastCopy(src.values, src.values + numElements(), values);
      return *this;
    }
    ...
    void appendRows(long num_rows_to_append)
    {
      ...
      if (!this->isEmpty())
      {
        ...
        fastCopy(values, values + old_num_elems, new_values);
    ...
    void invert()
    {
      ...
      std::vector<long> col_index((size_t)numRows()), row_index((size_t)numRows()), pivot((size_t)numRows());
