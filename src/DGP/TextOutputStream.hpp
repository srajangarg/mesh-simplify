...
#ifndef __DGP_TextOutputStream_hpp__
#define __DGP_TextOutputStream_hpp__
 ...
 */
class DGP_API TextOutputStream : public virtual NamedObject, private Noncopyable
{
  public:
    /**
    ...
    struct DGP_API WordWrap
    {
      ...
      DGP_ENUM_CLASS_BODY(WordWrap)
    };

    /** Newline style (enum class). */
    struct DGP_API NewlineStyle
    {
      enum Value
      {
        WINDOWS,
      ...
      DGP_ENUM_CLASS_BODY(NewlineStyle);
    };

    /** Output configuration optionss. */
    class DGP_API Settings
    {
      public:
...
#ifdef __GNUC__
#  define DGP_TEXTOUTPUTSTREAM_CHECK_PRINTF_ARGS   __attribute__((__format__(__printf__, 2, 3)))
#  define DGP_TEXTOUTPUTSTREAM_CHECK_VPRINTF_ARGS  __attribute__((__format__(__printf__, 2, 0)))
#else
#  define DGP_TEXTOUTPUTSTREAM_CHECK_PRINTF_ARGS
#  define DGP_TEXTOUTPUTSTREAM_CHECK_VPRINTF_ARGS
#endif
     ...
     */
    void __cdecl printf(char const * fmt, ...) DGP_TEXTOUTPUTSTREAM_CHECK_PRINTF_ARGS;
     ...
     * reformatted for word-wrapping and newlines.
     */
    void __cdecl vprintf(char const * fmt, va_list argPtr) DGP_TEXTOUTPUTSTREAM_CHECK_VPRINTF_ARGS;

#undef DGP_TEXTOUTPUTSTREAM_CHECK_PRINTF_ARGS
#undef DGP_TEXTOUTPUTSTREAM_CHECK_VPRINTF_ARGS
