...
#ifndef __DGP_VectorN_hpp__
#define __DGP_VectorN_hpp__
...
class /* DGP_DLL_LOCAL */ VectorNBase
{
  public:
    ...
    template <typename S> typename std::enable_if< ScalarCheck<S, T>::value, VectorT >::type operator*(S const & s) const
    {
      VectorT result;
      for (long i = 0; i < N; ++i)
    ...
    template <typename S> typename std::enable_if< ScalarCheck<S, T>::value, VectorT >::type & operator*=(S const & s)
    {
      for (long i = 0; i < N; ++i)
 ...
 */
template <long N, typename T>
class /* DGP_API */ VectorN : public Internal::VectorNBase<N, T>
{
  private:
...
typename std::enable_if< Internal::ScalarCheck<S, T>::value, VectorN<N, T> >::type
