...
void
parseCommaSeparated(std::string const & s, std::vector<std::string> & array, bool strip_quotes)
{
  array.clear();
...
long
stringSplit(std::string const & s, char split_char, std::vector<std::string> & result, bool skip_empty_fields)
{
  return stringSplit(s, std::string(1, split_char), result, skip_empty_fields);
}

long
stringSplit(std::string const & s, std::string const & split_chars, std::vector<std::string> & result, bool skip_empty_fields)
{
  result.clear();
...
std::string
stringJoin(std::vector<std::string> const & a, char join_char)
{
  std::ostringstream out;

  for (long i = 0; i < (long)a.size() - 1; ++i)
  {
    out << a[(size_t)i] << join_char;
  }
...
std::string
stringJoin(std::vector<std::string> const & a, std::string const & join_str)
{
  std::ostringstream out;
...
#define DGP_EOS            '\0'
#define DGP_RANGE_MATCH      1
#define DGP_RANGE_NOMATCH    0
#define DGP_RANGE_ERROR    (-1)
...
rangeMatch(char const * pattern, char test, int flags, char ** newp)
{
  ...
  do
  {
    ...
    if (c == DGP_EOS)
      return (DGP_RANGE_ERROR);

    if (c == '/' && (flags & FNM_PATHNAME))
      return (DGP_RANGE_NOMATCH);

    if ((flags & FNM_CASEFOLD))
      c = ::tolower((unsigned char)c);

    if (*pattern == '-'
        && (c2 = *(pattern + 1)) != DGP_EOS && c2 != ']')
    {
      pattern += 2;
  ...
  *newp = (char *)pattern;
  return (ok == negate ? DGP_RANGE_NOMATCH : DGP_RANGE_MATCH);
}
  ...
  for (stringstart = query;;)
  {
    switch (c = *pattern++)
    {
      case DGP_EOS:
        ...
        return (*query == DGP_EOS ? 0 : FNM_NOMATCH);
      ...
      case '[':
        ...
        {
          case DGP_RANGE_ERROR:
            /* not a good range, treat as normal text */
            goto fnmatch_normal;

          case DGP_RANGE_MATCH:
            pattern = newp;
            break;

          case DGP_RANGE_NOMATCH:
            return (FNM_NOMATCH);
        }
