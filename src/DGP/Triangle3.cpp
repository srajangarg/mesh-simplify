...
#define DGP_TRIANGLE_AREA2(d, e, f)  (((e)[i] - (d)[i]) * ((f)[j] - (d)[j]) - ((f)[i] - (d)[i]) * ((e)[j] - (d)[j]))

  // Area of the polygon
  Real area = DGP_TRIANGLE_AREA2(v0, v1, v2);
  if (area == 0)
  {
  ...
  b[0] = DGP_TRIANGLE_AREA2(p, v1, v2) * inv_area;
  if ((b[0] < 0) || (b[0] > 1))
    return false;

  b[1] = DGP_TRIANGLE_AREA2(v0, p, v2) * inv_area;
  if ((b[1] < 0) || (b[1] > 1))
    return false;

  b[2] = 1.0f - b[0] - b[1];

#undef DGP_TRIANGLE_AREA2
