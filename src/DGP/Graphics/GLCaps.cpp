...
// ImageFormats.
static std::map<Texture::Format const *, bool> _supportedTextureFormat;
static std::map<Texture::Format const *, bool> _supportedRenderBufferFormat;
...
GLCaps::createHeadlessContext()
{
  ...
  if (!headless_context)
  {
    DGP_ERROR << "GLCaps: Could not create OSMesa context";
    return false;
  }
  ...
  if (!headless_buffer)
  {
    DGP_ERROR << "GLCaps: Could not allocate dummy OSMesa offscreen buffer";
    return false;
  }

  if (!OSMesaMakeCurrent(headless_context, headless_buffer, GL_UNSIGNED_BYTE, DUMMY_FB_WIDTH, DUMMY_FB_HEIGHT))
  {
    DGP_ERROR << "GLCaps: Could not make new OSMesa context current";
    return false;
  }
  ...
  if (!headless_display)
  {
    DGP_ERROR << "GLCaps: Could not open X display";
    return false;
  }
  ...
  if (!vi)
  {
    DGP_ERROR << "GLCaps: Could not choose X visual";
    return false;
  }
  ...
  if (!headless_context)
  {
    DGP_ERROR << "GLCaps: Could not create X context";
    return false;
  }
  ...
  {
    DGP_ERROR << "GLCaps: Could not make new X context current";
    return false;
  }
  ...
  {
    DGP_ERROR << "GLCaps: Could not choose CGL pixel format (error: " << CGLErrorString(err) << ')';
    return false;
  }
  ...
  {
    DGP_ERROR << "GLCaps: Could not create CGL context (error: " << CGLErrorString(err) << ')';
    return false;
  }

  if ((err = CGLSetCurrentContext(headless_context)) != kCGLNoError)
  {
    DGP_ERROR << "GLCaps: Could not make new CGL context current (error: " << CGLErrorString(err) << ')';
    return false;
  }
...
GLCaps::loadExtensions()
{
  ...
  {
      DGP_WARNING << "This ATI Radeon Mobility card has a known bug with cube maps. Put cube map texture coordinates in the"
  ...
  if (DGP_SUPPORTS(NV_fragment_program))
  {
// To make this compile in the absence of the extension
  ...
  {
    // No multitexture
    DGP_DEBUG << "No GL_ARB_multitexture support: forcing number of texture units to no more than 1";
...
GLCaps::supportsTexture(Texture::Format const * fmt)
{
  ...
  std::map<Texture::Format const *, bool>::iterator existing = _supportedTextureFormat.find(fmt);
  if (existing == _supportedTextureFormat.end())
  {
    bool supportsFormat = false;

    if (!(fmt->floatingPoint && !DGP_SUPPORTS(ARB_texture_float)))
    {
...
{
  // First, check if we've already tested this format
  std::map<Texture::Format const *, bool>::iterator existing = _supportedRenderBufferFormat.find(fmt);
  if (existing == _supportedRenderBufferFormat.end())
  {
    bool supportsFormat = false;

    if (DGP_SUPPORTS(EXT_framebuffer_object) && !(fmt->floatingPoint && !DGP_SUPPORTS(ARB_texture_float)))
    {
...
{
  return DGP_SUPPORTS(ATI_separate_stencil) || DGP_SUPPORTS(EXT_stencil_two_side);
}

void
GLCaps::checkBug_cubeMapBugs()
{
  if (!DGP_SUPPORTS(ARB_texture_cube_map))
  {
    // No cube map == no bug
...
GLCaps::checkBug_mipmapGeneration()
{
  ...
  bug_mipmapGeneration = DGP_SUPPORTS(SGIS_generate_mipmap) && (beginsWith(r, "MOBILITY RADEON 90")
